...
use arkuuu\Publin\KeywordRepository;
use arkuuu\Publin\OAI\Exceptions\BadArgumentException;
use arkuuu\Publin\OAI\Exceptions\BadResumptionTokenException;
use arkuuu\Publin\OAI\Exceptions\BadVerbException;
use arkuuu\Publin\OAI\Exceptions\CannotDisseminateFormatException;
use arkuuu\Publin\OAI\Exceptions\IdDoesNotExistException;
use arkuuu\Publin\OAI\Exceptions\NoMetadataFormatsException;
use arkuuu\Publin\OAI\Exceptions\NoRecordsMatchException;
use arkuuu\Publin\OAI\Exceptions\NoSetHierarchyException;
use arkuuu\Publin\Publication;
use arkuuu\Publin\PublicationRepository;
...
class OAIParser
{
    ...
    public function listSets($resumptionToken = null)
    {
        ...
        if ($newCursor < $completeListSize) {
            ...
            $resumptionToken_element->setAttribute('expirationDate',
                date('Y-m-d\TH:i:s\Z', strtotime('+'.$this->token_valid_days.' days')));
    ...
    public function listIdentifiers($metadataPrefix, $from = null, $until = null, $set = null, $resumptionToken = null)
    {
        ...
        if ($newCursor < $completeListSize) {
            $newToken = $this->storeResumptionToken($metadataPrefix, $from, $until, $set, $newCursor,
                $completeListSize);
            $resumptionToken_element = $xml->createElement('resumptionToken', $newToken);
            $resumptionToken_element->setAttribute('expirationDate',
                date('Y-m-d\TH:i:s\Z', strtotime('+'.$this->token_valid_days.' days')));
    ...
    private function createRecordHeader(Publication $publication)
    {
        ...
        $header->appendChild($xml->createElement('identifier',
            'oai:'.$this->repositoryIdentifier.':'.$publication->getId()));
    ...
    private function createRecordMetadata(Publication $publication, $metadataPrefix)
    {
        ...
        $oai_dc->setAttribute('xsi:schemaLocation',
    ...
    private function createRecordMetadataFields(Publication $publication, $metadataPrefix)
    {
        if ($metadataPrefix == 'oai_dc') {
            ...
            $fields[] = array(
                'dc:identifier',
                Request::createUrl(array('p' => 'publication', 'id' => $publication->getId())),
            );
